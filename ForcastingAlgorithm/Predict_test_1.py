"""
第一代：仅对未来进行预测，未来数据、预测数据均不使用，且将选取
"""
...
predict_type = 1
paramter = 5
Threshold = 365   # 150 MAX
sigma = math.sqrt(0.5)   # 0.5 0.5
mu = 0.95   #0.95 0.995
gamma = 0.126   # 0.126 0.126
SIM_range = 1   # 1 1
...
for date_index in range(len(date_predict)):
    ...
    for i in range(len(SIMCount)):
        if SIMCount[i] >= (max(SIMCount) - SIM_range):
            ...
            SumPowerLoadMax_pridict += (PowerLoadMax[i] * (1 + gamma * float(IntervelDays / 365)))
...
yticks = range(1000,10000,1000)
ytickslabel = range(1000,10000,1000)
