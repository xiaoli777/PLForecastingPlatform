...
def Predict_Main(date_start="2007-1-1", date_end="2007-12-31", paramter = 7, predicttype="Max"):
    ...
    for date_index in range(len(date_predict)):
        ...
        season = " and date in (select date from date67 where season = (select season from date67 where date = '" + \
                  date_predict[date_index] + "'))"
        holiday = " and date in (select date from date67 where holiday = (select holiday from date67 where date = '" + \
                  date_predict[date_index] + "') and week >= (select week from date67 where date = '" + \
                  date_predict[date_index] + "'))"
