...
def Predict_Main(date_start="2007-1-1", date_end="2007-12-31", paramter = 7, predicttype="Max"):
    ...
    for date_index in range(len(date_predict)):
        ...
        season = " and date in (select date from datew2h where season = (select season from datew2h where date = '" + \
                 date_predict[date_index] + "'))"
        holiday = " and date in (select date from datew2h where holiday = (select holiday from datew2h where date = '" + \
                  date_predict[date_index] + "'))"
        finish_signal = ";"
        ...
        for mu_index in range(len(muList)):
            for sigma_index in range(len(sigmaList)):
                ...
                clf = svm.SVR(gamma='auto', C=100, epsilon=50)
                clf.fit(SVM_X, SVM_y)
