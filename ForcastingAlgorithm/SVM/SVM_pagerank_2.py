...
def train_attribute(date_list, name):
    conn = pymysql.connect(host='localhost', port=3306, user='yxl', passwd='123456', db='powerload')  # db：库名
    cur = conn.cursor()
    datalist = []
    ...
    for date in date_list:
        current_data = []
        Count = cur.execute('select ' + name + ' from date '
                                               'where date = "' + date + '";')
...
def train_select(date, predict_type, paramter=1):
    ...
    # holiday = train_attribute(data_history,"holiday")
    # week = train_attribute(data_history,"week")
    # season = train_attribute(data_history,"season")
    #
    ...
    # samplein = np.mat([AverTemper, AverPress, AverSPress, LowTemper, HighTemper, LowPress, HighPress, history_1, history_2, history_3, history_4, history_5, history_6, history_7,holiday_1,holiday_2,holiday_3,week_1,week_2,week_3])
...
def Predict_Main(date_start="2007-1-1", date_end="2007-12-31", paramter = 5, predicttype="Max"):
    ...
    for date_index in range(len(date_predict)):
        ...
        current_predict_result = algorithm_main(SVM_X, SVM_y)
