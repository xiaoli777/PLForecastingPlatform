...
SIM_range = 0
sigmaList = [math.sqrt(0.5),math.sqrt(0.75),math.sqrt(1)]
muList = [0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.94,0.95,0.98]
MaxValue = 10000
...
def Predict_Main(date_start="2007-1-1", date_end="2007-12-31", paramter = 7, predicttype="Max"):
    ...
    for date_index in range(len(date_predict)):
        ...
        for mu_index in muList:
            for sigma_index in sigmaList:
                ...
                for row in range(sample_temp.shape[0]):
                    for column in range(sample_temp.shape[1]):
                        if np.exp(-(sample_temp[row][column] ** 2) / (2 * sigma_index * sigma_index)) >= mu_index:
                ...
                #print(abs(predict_result - PowerLoadMax_real[date_index]),predict_finalresult)
        ...
        SVM_result.append(predict_finalresult)
        #print(SVM_result[date_index],PowerLoadMax_real[date_index])
