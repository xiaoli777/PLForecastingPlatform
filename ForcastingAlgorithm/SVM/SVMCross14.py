...
muList = [0.3,0.325,0.35,0.375,0.4,0.425,0.45,0.475,0.5,0.525,0.55,0.575,0.6,0.625,0.65,0.675,0.7,0.725,0.75,0.775,0.8,0.825,0.85,0.875,0.9,0.925,0.94,0.95,0.975,0.98,0.985,0.99]
#muList = [0.5,0.525,0.55,0.575,0.6,0.625,0.65,0.675,0.7,0.725,0.75,0.775,0.8,0.825,0.85,0.875,0.9,0.925,0.94,0.95,0.975,0.98,0.985,0.99]
MaxValue = 10000
...
def Predict_Main(date_start="2007-1-1", date_end="2007-12-31", paramter = 7, predicttype="Max"):
    ...
    for date_index in range(len(date_predict)):
        ...
        history_8 = pre_week(data_history, predict_type, 8)
        history_9 = pre_week(data_history, predict_type, 9)
        history_10 = pre_week(data_history, predict_type, 10)
        history_11 = pre_week(data_history, predict_type, 11)
        history_12 = pre_week(data_history, predict_type, 12)
        history_13 = pre_week(data_history, predict_type, 12)
        history_14 = pre_week(data_history, predict_type, 14)
        ...
        current_data.append(history_5[len(history_8) - 1])
        current_data.append(history_9[len(history_9) - 1])
        current_data.append(history_10[len(history_10) - 1])
        current_data.append(history_11[len(history_11) - 1])
        current_data.append(history_12[len(history_12) - 1])
        current_data.append(history_13[len(history_13) - 1])
        current_data.append(history_14[len(history_14) - 1])


        #print(current_data)

        samplein = np.mat([AverTemper, AverPress, AverSPress, LowTemper, HighTemper, LowPress, HighPress, history_1, history_2, history_3, history_4, history_5, history_6, history_7, history_8, history_9, history_10, history_11, history_12, history_13, history_14])
        #print(samplein)
                    ...
                    if SIMCount[i] >= (max(SIMCount) - SIM_range):
                        ...
                        SVM_X.append(
                            [AverTemper[i], AverPress[i], AverSPress[i], LowTemper[i], HighTemper[i], LowPress[i],
                             ...
                             history_6[i], history_7[i], history_8[i], history_9[i], history_10[i], history_11[i],
                             history_12[i], history_13[i], history_14[i]])
