...
# print(sampleinminmax.transpose()[1])   #T、transpose都是转置矩阵
#2*20
...
w1 = 0.5*np.random.rand(hiddenunitnum,indim)-0.1   #rand(num,num)   random((num,num))
b1 = 0.5*np.random.rand(hiddenunitnum,1)-0.1    #hiddenunit内部的值
...
for i in range(maxepochs):
    ...
    delta1 = np.dot(w2.transpose(),delta2)*hiddenout*(1-hiddenout)   #np.dot矩阵相乘
