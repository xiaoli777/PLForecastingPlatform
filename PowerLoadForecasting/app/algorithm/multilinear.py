"""
第四代：仅对未来进行预测，使用未来数据，不使用预测数据,对一些异常点单独测试，模块化，linear预测模型
"""
import pymysql
import numpy as np
import math
...
def Predict_Main(date_start="2007-1-1", date_end="2007-1-1", paramter = 5, predicttype="Max"):
    ...
    for date_index in range(len(date_predict)):
        ...
        regr = linear_model.LinearRegression()
        regr.fit(linear_X, linear_y)
        current_predict_result = round(float(regr.predict(current_data)), 2)

        linear_result.append(current_predict_result)
        pre_best_i = current_best_i
        pre_best_j = current_best_j

    # 计算MAPE\MASE
    MAPE = CalMAPE(PowerLoadMax_real, linear_result)
    #MASE = CalMASE(PowerLoadMax_real, linear_result)
    #print("MAPE:" + str(round(MAPE * 100,2)) + "%")
    #print("MASE:" + str(round(MASE * 100, 2)) + "%")
    return PredictList(date_predict, PowerLoadMax_real, linear_result, MAPE)
